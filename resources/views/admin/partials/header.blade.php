        ...
        <li class="nav-item dropdown">
            <a role="button" class="dropdown-toggle nav-link">
                <span>
                    ...
                    @elseif(Auth::guard(config('zekini-admin.defaults.guard'))->check() && Auth::guard(config('zekini-admin.defaults.guard'))->user()->first_name && Auth::guard(config('zekini-admin.defaults.guard'))->user()->last_name)
                        <span class="avatar-initials">{{ mb_substr(Auth::guard(config('zekini-admin.defaults.guard'))->user()->first_name, 0, 1) }}{{ mb_substr(Auth::guard(config('zekini-admin.defaults.guard'))->user()->last_name, 0, 1) }}</span>
                    @else
                        <span class="avatar-initials"><i class="fa fa-user"></i></span>
                    @endif

                    @if(!is_null(config('zekini-admin.defaults.guard')))
                        <span class="hidden-md-down">{{ Auth::guard(config('zekini-admin.defaults.guard'))->check() ? Auth::guard(config('zekini-admin.defaults.guard'))->user()->full_name : 'Anonymous' }}</span>
                    @else
